...
class AttributeUpdate extends AbstractUpdate implements Observer 
{
	...
	var attributeMap: Map<String, AttributeStrategyI> = [
	...
	public function buildAttributes(component: Component) : Void {
		...
		for (attribute in component.getAttributes()) {
			// change
			//var attributeValue = component.getUntyped(attribute).getType().getUid();
			//if (attributeValue == "IntegerAttributeValue" || attributeValue == "OrientationAttributeValue" || attributeValue == "StringAttributeValue") {
				var attributeValue = component.getUntyped(attribute);
				var strategy = this.attributeMap[attributeValue.getType().getUid()];
