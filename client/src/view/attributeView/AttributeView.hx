 ...
 */
class AttributeView implements Observer implements AttributeViewI
{
	...
	public function buildAttributes(selectionModel: SelectionModel, refresh: Bool) : Void {
		...
		for (attribute in this.currentAttributeSet) {
			...
			if( strategy != null ) {
				this.attributePane.appendChild(strategy.spawnHTMLAttribute(attribute.getAttributeUntyped(), attribute.getAttributeValue(), attribute.getAttributeStatus(), selectionModel, this.view));
