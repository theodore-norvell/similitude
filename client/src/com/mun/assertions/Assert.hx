...
/** Assertions. Based on code by Simon Krajewski.
* See https://code.haxe.org/category/macros/assert-with-values.html
*
**/
class Assert {
    static public macro function assert(e:Expr) {
        ...
        el.push(macro if (!$e) {
            var e = new AssertionFailure($v{s}, $a{a}) ;
            @:pos(p) trace("Throwing exception "+e) ;
            @:pos(p) throw e;});
        return macro $b{el};
    }
}
...
class AssertionFailure {
    ...
    public function toString() {
        ...
        buf.add("Assertion failure: " + message + ", where");
        for (part in parts) {
