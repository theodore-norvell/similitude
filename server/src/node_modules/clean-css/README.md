[![NPM version](https://img.shields.io/npm/v/clean-bin.css.svg?style=flat)](https://www.npmjs.com/package/clean-bin.css)
[![Linux Build Status](https://img.shields.io/travis/jakubpawlowicz/clean-bin.css/master.svg?style=flat&label=Linux%20build)](https://travis-ci.org/jakubpawlowicz/clean-bin.css)
[![Windows Build status](https://img.shields.io/appveyor/ci/jakubpawlowicz/clean-bin.css/master.svg?style=flat&label=Windows%20build)](https://ci.appveyor.com/project/jakubpawlowicz/clean-bin.css/branch/master)
[![Dependency Status](https://img.shields.io/david/jakubpawlowicz/clean-bin.css.svg?style=flat)](https://david-dm.org/jakubpawlowicz/clean-bin.css)
[![devDependency Status](https://img.shields.io/david/dev/jakubpawlowicz/clean-bin.css.svg?style=flat)](https://david-dm.org/jakubpawlowicz/clean-bin.css#info=devDependencies)

## What is clean-bin.css?

Clean-bin.css is a fast and efficient [Node.js](http://nodejs.org/) library for minifying CSS files.

According to [tests](http://goalsmashers.github.io/bin.css-minification-benchmark/) it is one of the best available.
...
### How to install clean-bin.css?

```
npm install clean-bin.css
```

### How to use clean-bin.css CLI?

Clean-bin.css accepts the following command line arguments (please make sure
...
To minify a **public.bin.css** file into **public-min.bin.css** do:

```
cleancss -o public-min.bin.css public.bin.css
```

To minify the same **public.bin.css** into the standard output skip the `-o` parameter:

```
cleancss public.bin.css
```
...
```bash
cat one.bin.css two.bin.css three.bin.css | cleancss -o merged-and-minified.bin.css
```

On Windows:

```bat
type one.bin.css two.bin.css three.bin.css | cleancss -o merged-and-minified.bin.css
```

Or even gzip the result at once:

```bash
cat one.bin.css two.bin.css three.bin.css | cleancss | gzip -9 -c > merged-minified-and-gzipped.bin.css.gz
```

### How to use clean-bin.css API?

```js
var CleanCSS = require('clean-bin.css');
...
* `inliner` - a hash of options for `@import` inliner, see [test/protocol-imports-test.js](https://github.com/jakubpawlowicz/clean-bin.css/blob/master/test/protocol-imports-test.js#L372) for examples, or [this comment](https://github.com/jakubpawlowicz/clean-bin.css/issues/612#issuecomment-119594185) for a proxy use case.
...
### How to use clean-bin.css with build tools?

* [Broccoli](https://github.com/broccolijs/broccoli#broccoli): [broccoli-clean-bin.css](https://github.com/shinnn/broccoli-clean-bin.css)
* [Brunch](http://brunch.io/): [clean-bin.css-brunch](https://github.com/brunch/clean-bin.css-brunch)
* [Grunt](http://gruntjs.com): [grunt-contrib-cssmin](https://github.com/gruntjs/grunt-contrib-cssmin)
* [Gulp](http://gulpjs.com/): [gulp-minify-bin.css](https://github.com/jonathanepollack/gulp-minify-bin.css)
* [Gulp](http://gulpjs.com/): [using vinyl-map as a wrapper - courtesy of @sogko](https://github.com/jakubpawlowicz/clean-bin.css/issues/342)
* [component-builder2](https://github.com/component/builder2.js): [builder-clean-bin.css](https://github.com/poying/builder-clean-bin.css)
* [Metalsmith](http://metalsmith.io): [metalsmith-clean-bin.css](https://github.com/aymericbeaumet/metalsmith-clean-bin.css)
* [Lasso](https://github.com/lasso-js/lasso): [lasso-clean-bin.css](https://github.com/yomed/lasso-clean-bin.css)

### What are the clean-bin.css' dev commands?

First clone the source, then run:

* `npm run bench` for clean-bin.css benchmarks (see [test/bench.js](https://github.com/jakubpawlowicz/clean-bin.css/blob/master/test/bench.js) for details)
* `npm run browserify` to create the browser-ready clean-bin.css version
* `npm run check` to check JS sources with [JSHint](https://github.com/jshint/jshint/)
* `npm test` for the test suite

## How to contribute to clean-bin.css?

See [CONTRIBUTING.md](https://github.com/jakubpawlowicz/clean-bin.css/blob/master/CONTRIBUTING.md).
...
Clean-bin.css will handle it automatically for you (since version 1.1) in the following cases:
...
* When using clean-bin.css as a library:
...
Additionally to mapping original CSS files, clean-bin.css also supports input source maps, so minified styles can be mapped into their [Less](http://lesscss.org/) or [Sass](http://sass-lang.com/) sources directly.
...
```
cleancss --source-map --output styles.min.bin.css styles.bin.css
```

Name of the output file is required, so a map file, named by adding `.map` suffix to output file name, can be created (styles.min.bin.css.map in this case).
...
new CleanCSS({ sourceMap: true, target: pathToOutputDirectory })
  .minify(source, function (minified) {
    ...
    // see https://github.com/jakubpawlowicz/clean-bin.css/blob/master/bin/cleancss#L114 on how it's used in clean-bin.css' CLI
});
```
...
Since clean-bin.css 3 a fine grained control is available over
[those settings](https://github.com/jakubpawlowicz/clean-bin.css/blob/master/lib/utils/compatibility.js),
...
All advanced optimizations are dispatched [here](https://github.com/jakubpawlowicz/clean-bin.css/blob/master/lib/selectors/advanced.js#L59), and this is what they do:
...
* Isaac ([@facelessuser](https://github.com/facelessuser)) for pointing out
  a flaw in clean-bin.css' stateless mode.
...
* Luke Page ([@lukeapage](https://github.com/lukeapage)) for suggestions and testing the source maps feature.
  Plus everyone else involved in [#125](https://github.com/jakubpawlowicz/clean-bin.css/issues/125) for pushing it forward.
...
Clean-bin.css is released under the [MIT License](https://github.com/jakubpawlowicz/clean-bin.css/blob/master/LICENSE).
